...
- When a destructive command (delete, drop, truncate, or similar) targets a production-flagged connection, the confirm step in docs/05-confirmation-workflow.md requires one additional action: the user types the exact affected record count shown in the preview, or types the word CONFIRM, before the confirm button becomes active.
